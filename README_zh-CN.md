...
3. **工作流集成**：
   ...
   - **⚠️ 重要提示**：由于 Codex prompts 限制，`/workflow` 命令无法直接传递参数。请先使用 `/workflow`，然后在后续消息中提供任务描述
...
> **⚠️ Codex 用户重要提示**：由于 Codex 功能限制，prompts 无法传递参数。使用 `/workflow` 工作流时，需要先发出 `/workflow` 命令，等 AI 回复后，再发送您的需求说明。
